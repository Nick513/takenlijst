...
@section('content')
    <div class="container h-100">
        ...
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a class="nav-link active" aria-current="page" href="javascript:void(0);" id="nav-general-tab"
                   data-bs-toggle="tab" data-bs-target="#nav-general" type="button" role="tab"
                   aria-controls="nav-general" aria-selected="true">{{ __('General') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="javascript:void(0);" id="nav-api-tab" data-bs-toggle="tab"
                   data-bs-target="#nav-api" type="button" role="tab" aria-controls="nav-api"
                   aria-selected="true">{{ __('API') }}</a>
            </li>
        </ul>
                    ...
                    <div class="card-body">
                        ...
                        <div class="card-select">
                            <select class="form-select" name="amountOfTasks">
                                @for ($i = 3; $i < 12; $i++)
                                    <option @if($user['settings'] !== null && json_decode($user['settings'], true)['tasks']['amount'] === $i+1) selected
                                            @endif value="{{ $i+1 }}">{{ $i+1 }}</option>
                                @endfor
                            </select>
                        </div>
